...
def save_postgresql_versions_to_sqlite(osver):
    ...
    for line in vers:
        ...
        if osver.startswith('el'):
            ...
            pkgs = [
                "{}-{}PGDG.{}.x86_64".format(pkg, ver, osname) for pkg in pkgs]
            ...
            project = (
                ...
                'x86_64',
                repo_url,
                '/etc/yum.repos.d/percona-ppg-{ver_no_dot}-release.repo'.format(
                    ver_no_dot=ver_no_dot),
                'ppg-{ver_no_dot}'.format(ver_no_dot=ver_no_dot),
                'postgresql-{maj}'.format(maj=maj_ver),
                '|'.join(pkgs),
                'gdb|percona-postgresql{maj}-server-debuginfo-{ver}.{osver}.x86_64'.format(
                    maj=maj_ver, ver=ver, osver=osver)
            )
...
def save_percona_backup_mongodb_versions_to_sqlite(osver):
    ...
    for line in vers:
        ...
        project = ()
        if osver.startswith('el'):
            pbm_ver_short = re.sub(
                r'^([0-9]+\.[0-9]+\.[0-9]+)-.*$', r'\1', ver)
            pbm_repo_url = 'https://www.percona.com/downloads/percona-backup-mongodb/percona-backup-mongodb-{pbm_version_short}'.format(
                pbm_version_short=pbm_ver_short)
            project = (
                ver, osver, arch,
...
def save_percona_server_mongodb_versions_to_sqlite(osver):
    ...
    for arch in ('x86_64', 'aarch64'):
        vers_file = ".version-info/psmdb.{os}.{arch}.txt".format(
            os=osver, arch=arch)
      ...
      """
        for line in vers:
            ...
            project = ()
            if osver.startswith('el'):
                pkgs = ['percona-server-mongodb-tools', 'percona-server-mongodb-server',
                        'percona-server-mongodb-mongos', 'percona-server-mongodb']
                pkgs = ["{}-{}.{}.{}".format(pkg, ver, osver, arch)
                        for pkg in pkgs]
                if ver.startswith('6.0') or ver.startswith('7.0'):
                    mongoshver = '1.6.1-1'
                    if ver.startswith('6.0.2'):
                        mongoshver = '1.6.0-1'
                        pkgs.insert(1, 'percona-mongodb-mongosh-{mongoshver}.{osver}.{arch}'.format(
                            mongoshver=mongoshver, osver=osver, arch=arch))
                    else:
                        pkgs.insert(1, 'percona-mongodb-mongosh')
                else:
                    pkgs.insert(
                        1, 'percona-server-mongodb-shell-{ver}.{osver}.{arch}'.format(ver=ver, osver=osver, arch=arch))

                ver_no_dot = ''.join(ver.split('.', 2)[0:2])
                project = (
                    ver, osver, arch,
                    'http://repo.percona.com/yum/percona-release-latest.noarch.rpm',
                    '/etc/yum.repos.d/percona-psmdb-{ver_no_dot}-release.repo'.format(
                        ver_no_dot=ver_no_dot),
                    'psmdb-{ver_no_dot}'.format(
                        ver_no_dot=ver_no_dot), 'mongod', '/etc/mongod.conf',
                    '|'.join(pkgs),
                    'gdb|percona-server-mongodb-debuginfo-{ver}.{osver}.{arch}'.format(
...
def save_mysql_server_versions_to_sqlite(osver):
    ...
    for arch in ("x86_64", "aarch64"):
        ...
        if arch == 'aarch64':
            vers_file = ".version-info/mysql.{os}.{arch}.txt".format(
                os=osver, arch=arch)
        if not os.path.exists(vers_file):
            continue
        vers = list(open(vers_file))
      ...
      """
        for line in vers:
            ...
            project = ()
            if osver.startswith('el'):
                pkgs = ["mysql-community-common", "mysql-community-libs",
                        "mysql-community-client", "mysql-community-server"]
                dbg_pkg = ['gdb']
                if ver.startswith('8.0'):
                    dbg_pkg.append(
                        'https://cdn.mysql.com//Downloads/MySQL-8.0/mysql-community-debuginfo-{ver}.{osver}.{arch}.rpm'.format(ver=ver, osver=osver, arch=arch))
                if ver.startswith('5.7'):
                    dbg_pkg.append(
                        'https://cdn.mysql.com//Downloads/MySQL-5.7/mysql-community-debuginfo-{ver}.{osver}.{arch}.rpm'.format(ver=ver, osver=osver, arch=arch))
                if ver.startswith('5.6'):
                    dbg_pkg.append(
                        'https://cdn.mysql.com//Downloads/MySQL-5.6/mysql-community-debuginfo-{ver}.{osver}.{arch}.rpm'.format(ver=ver, osver=osver, arch=arch))
                if osver == 'el7':
                ...
                mysql_shell_pkg = '{url}-{ver}.{osver}.{arch}.rpm'.format(
                    url=mysql_shell_pkg, ver=ver, osver=osver, arch=arch)
                project = (
                    ...
                    '|'.join(["{}-{}.{}.{}".format(pkg, ver, osver, arch)
                             for pkg in pkgs]),
                    '|'.join(dbg_pkg),
                    'mysql-community-test-{ver}.{osver}.{arch}'.format(
                        ver=ver, osver=osver, arch=arch),
                    mysql_shell_pkg,
                    'mysql-router-community-{ver}.{osver}.{arch}'.format(
                        ver=ver, osver=osver, arch=arch)
                )
...
def save_percona_xtradb_cluster_versions_to_sqlite(osver):
    ...
    cur = conn.cursor()
    vers = list(
        open(".version-info/percona-xtradb-cluster.{os}.txt".format(os=osver)))
    sql = """\
    ...
    for line in vers:
        ...
        if ver.startswith('8.0') and osver.startswith('el'):
            pkgs = ['percona-xtradb-cluster-shared',
            ...
            project = (
                ...
                '|'.join(pkgs),
                'gdb|percona-xtradb-cluster-debuginfo-{ver}.{osver}.x86_64'.format(
                    ver=ver, osver=osver),
                'percona-xtradb-cluster-test-{ver}.{osver}.x86_64'.format(
                    ver=ver, osver=osver),
                'percona-xtradb-cluster-garbd-{ver}.{osver}.x86_64'.format(
                    ver=ver, osver=osver)
            )
        elif ver.startswith('5.7') and osver.startswith('el'):
            pkgs = ['Percona-XtraDB-Cluster-shared-57',
                    'Percona-XtraDB-Cluster-client-57', 'Percona-XtraDB-Cluster-server-57']
            if osver == 'el7':
            ...
            project = (
                ...
                '|'.join(["{}-{}.{}.x86_64".format(pkg, ver, osver)
                          for pkg in pkgs]),
                'gdb|Percona-XtraDB-Cluster-57-debuginfo-{ver}.{osver}.x86_64'.format(
                    ver=ver, osver=osver),
                'Percona-XtraDB-Cluster-test-57-{ver}.{osver}.x86_64'.format(
                    ver=ver, osver=osver),
                'Percona-XtraDB-Cluster-garbd-57-{ver}.{osver}.x86_64'.format(
                    ver=ver, osver=osver)
            )
        elif ver.startswith('5.6') and osver.startswith('el'):
            pkgs = ['Percona-XtraDB-Cluster-shared-56',
                    'Percona-XtraDB-Cluster-client-56', 'Percona-XtraDB-Cluster-server-56']
            pkgs = ["{}-{}.{}.x86_64".format(pkg, ver, osver) for pkg in pkgs]
            pkgs.insert(0, 'which')
            ...
            project = (
                ...
                '|'.join(pkgs),
                'gdb|Percona-XtraDB-Cluster-56-debuginfo-{ver}.{osver}.x86_64'.format(
                    ver=ver, osver=osver),
                'Percona-XtraDB-Cluster-test-56-{ver}.{osver}.x86_64'.format(
                    ver=ver, osver=osver),
                'Percona-XtraDB-Cluster-garbd-56-{ver}.{osver}.x86_64'.format(
                    ver=ver, osver=osver)
            )
        if len(project) > 1:
            cur.execute(sql, project)
...
def save_mariadb_versions_to_sqlite(osver):
    ...
    for line in vers:
        ...
        if osver.startswith('el'):
            pkgs = ['MariaDB-common', 'MariaDB-shared',
                    'MariaDB-client', 'MariaDB-server']

            archive_url = 'https://mirror.mariadb.org/yum/{ver_short}/centos{elver}-amd64/rpms'.format(
                ver_short=ver_short, elver=osver.replace('el', ''))
            ...
            if osver.startswith('el7'):
                ...
                archive_url = 'https://mirror.mariadb.org/yum/{ver_short}/rhel{elver}-amd64/rpms'.format(
                    ver_short=ver_short, elver=osver.replace('el', ''))
            ...
            project = (
                ...
                '|'.join(["{url}/{pkg}-{ver}.{suf}".format(url=archive_url,
                                                           pkg=pkg, ver=ver, suf=pkg_suffix) for pkg in pkgs]),
                'gdb|{url}/MariaDB-server-debuginfo-{ver}.{suf}'.format(
                    ver=ver, url=archive_url, suf=pkg_suffix),
                '{url}/MariaDB-rocksdb-engine-{ver}.{suf}'.format(
                    url=archive_url, ver=ver, suf=pkg_suffix),
                '{url}/MariaDB-test-{ver}.{suf}'.format(
                    url=archive_url, ver=ver, suf=pkg_suffix),
                ''
            )
...
def save_general_version_for_program(vers_file, osrver, arch, program):
    vers_file = str(
        (Path(os.getcwd()) / ".version-info" / vers_file).resolve())
    if not os.path.exists(vers_file):
        return
    vers = list(open(vers_file))
    for line in vers:
...
"""
    ...
    for line in vers:
        ...
        if osver.startswith('el'):
            ...
            pkgs = ['percona-xtrabackup-{versuf}-{ver}.{pkg_suffix}'.format(
                versuf=versuf, ver=ver, pkg_suffix=pkg_suffix)]
...
def save_github_tags_to_sqlite(repo_name, tbl):
    ...
    try:
        ...
        return
    cur = conn.cursor()
    sql = "INSERT OR REPLACE INTO {tbl} (name,version) VALUES(?, ?)".format(
        tbl=tbl)
...
def save_percona_server_versions_to_sqlite(osver):
    ...
    for arch in ("x86_64", "aarch64"):
        ...
        if arch == 'aarch64':
            vers_file = ".version-info/percona-server.{os}.{arch}.txt".format(
                os=osver, arch=arch)
            ...
            if (ver.startswith('8.0') or ver.startswith('8.4') ) and osver.startswith('el'):
                ...
                project = (
                    ...
                    'gdb|percona-server-debuginfo-{ver}.{osver}.{arch}'.format(
                        ver=ver, osver=osver, arch=arch),
                    'percona-server-rocksdb-{ver}.{osver}.{arch}'.format(
                        ver=ver, osver=osver, arch=arch),
                ...
                pkgs = ['Percona-Server-shared-compat-57', 'Percona-Server-shared-57',
                        'Percona-Server-client-57', 'Percona-Server-server-57']
                project = (
                    ver, osver, arch,
                    'http://repo.percona.com/yum/percona-release-latest.noarch.rpm',
                    '/etc/yum.repos.d/percona-original-release.repo',
                    ...
                    '|'.join(["{}-{}.{}.{}".format(pkg, ver, osver, arch)
                             for pkg in pkgs]),
                    'gdb|Percona-Server-57-debuginfo-{ver}.{osver}.{arch}'.format(
                        ver=ver, osver=osver, arch=arch),
                    'Percona-Server-rocksdb-57-{ver}.{osver}.{arch}'.format(
                        ver=ver, osver=osver, arch=arch),
                    'Percona-Server-test-57-{ver}.{osver}.{arch}'.format(
                        ver=ver, osver=osver, arch=arch),
                    '', ''
                )
            elif ver.startswith('5.6') and osver.startswith('el'):
                pkgs = ['Percona-Server-shared-56',
                ...
                project = (
                    ...
                    '|'.join(["{}-{}.{}.x86_64".format(pkg, ver, osver)
                              for pkg in pkgs]),
                    'gdb|Percona-Server-56-debuginfo-{ver}.{osver}.x86_64'.format(
                        ver=ver, osver=osver),
                    'Percona-Server-rocksdb-56-{ver}.{osver}.x86_64'.format(
                        ver=ver, osver=osver),
                    'Percona-Server-test-56-{ver}.{osver}.x86_64'.format(
                        ver=ver, osver=osver),
                    '', ''
                )
            elif ver.startswith('5.5') and osver.startswith('el'):
                pkgs = ['Percona-Server-shared-55',
                ...
                project = (
                    ...
                    '|'.join(["{}-{}.{}.x86_64".format(pkg, ver, osver)
                              for pkg in pkgs]),
                    'gdb|Percona-Server-55-debuginfo-{ver}.{osver}.x86_64'.format(
                        ver=ver, osver=osver),
                    'Percona-Server-rocksdb-55-{ver}.{osver}.x86_64'.format(
                        ver=ver, osver=osver),
                    'Percona-Server-test-55-{ver}.{osver}.x86_64'.format(
                        ver=ver, osver=osver),
                    '', ''
                )
            if len(project) > 1:
...
def update_versions():
    ...
    generate_versions_file("mariadb.el8.txt",
                           [
                               ...
                               {"url": "https://mirror.mariadb.org/yum/10.4/rhel8-amd64/rpms/",
                                   "pattern": r'MariaDB-server-(\d[^"]*).el8.x86_64.rpm'},
                               {"url": "https://mirror.mariadb.org/yum/10.5/rhel8-amd64/rpms/",
                                   "pattern": r'MariaDB-server-(\d[^"]*).el8.x86_64.rpm'},
                               {"url": "https://mirror.mariadb.org/yum/10.6/rhel8-amd64/rpms/",
                                   "pattern": r'MariaDB-server-(\d[^"]*).el8.x86_64.rpm'},
                               {"url": "https://mirror.mariadb.org/yum/11.1/rhel8-amd64/rpms/",
                                   "pattern": r'MariaDB-server-(\d[^"]*).el8.x86_64.rpm'},
                               {"url": "https://mirror.mariadb.org/yum/11.2/rhel8-amd64/rpms/",
                                   "pattern": r'MariaDB-server-(\d[^"]*).el8.x86_64.rpm'},
                               {"url": "https://mirror.mariadb.org/yum/11.4/rhel8-amd64/rpms/",
                                   "pattern": r'MariaDB-server-(\d[^"]*).el8.x86_64.rpm'},
                               {"url": "https://mirror.mariadb.org/yum/11.5/rhel8-amd64/rpms/",
                                   "pattern": r'MariaDB-server-(\d[^"]*).el8.x86_64.rpm'},
                               {"url": "https://mirror.mariadb.org/yum/11.6/rhel8-amd64/rpms/",
                                   "pattern": r'MariaDB-server-(\d[^"]*).el8.x86_64.rpm'}
                           ])
    ...
    save_general_version_for_program(
        "percona-orchestrator.el8.txt", "el8", "x86_64", "percona-orchestrator")
    save_general_version_for_program(
        "percona-orchestrator.el9.txt", "el9", "x86_64", "percona-orchestrator")
    ...
    save_general_version_for_program(
        "pmm-client.el7.txt", "el7", "x86_64", "pmm-client")
    save_general_version_for_program(
        "pmm-client.el8.txt", "el8", "x86_64", "pmm-client")
    save_general_version_for_program(
        "pmm-client.el9.txt", "el9", "x86_64", "pmm-client")
    ...
    generate_versions_file("pg.el8.txt",
                           [
                               ...
                               {"url": "https://download.postgresql.org/pub/repos/yum/16/redhat/rhel-8-x86_64",
                                "pattern": r'postgresql\d+-server-([0-9.-]+)PGDG\.rhel\d+.x86_64.rpm'},
                               {"url": "https://download.postgresql.org/pub/repos/yum/17/redhat/rhel-8-x86_64",
                                "pattern": r'postgresql\d+-server-([0-9.-]+)PGDG\.rhel\d+.x86_64.rpm'},
                           ])
    ...
    generate_versions_file("pg.el9.txt",
                           [
                               ...
                               {"url": "https://download.postgresql.org/pub/repos/yum/16/redhat/rhel-9-x86_64",
                                "pattern": r'postgresql\d+-server-([0-9.-]+)PGDG\.rhel\d+.x86_64.rpm'},
                               {"url": "https://download.postgresql.org/pub/repos/yum/17/redhat/rhel-9-x86_64",
                                "pattern": r'postgresql\d+-server-([0-9.-]+)PGDG\.rhel\d+.x86_64.rpm'},
                           ])
